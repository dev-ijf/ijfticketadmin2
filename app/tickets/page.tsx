...
function ScannerHtml5Qrcode({
  ...
  onScan,
  onError,
}: { open: boolean; onScan: (text: string) => void; onError: (err: any) => void }) {
...
export default function TicketsPage() {
  ...
  const updateCheckInStatus = async (ticketId: number, isCheckedIn: boolean) => {
    try {
      const response = await fetch("/api/tickets", {
        ...
        body: JSON.stringify({ ticketId, isCheckedIn }),
      })
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    const matchesEvent = eventFilter === "all" || (ticket.event_id && ticket.event_id.toString() === eventFilter)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={exportToExcel} disabled={exportLoading || tickets.length === 0}>
          ...
          <Dialog open={scannerOpen} onOpenChange={setScannerOpen}>
            ...
            <DialogContent className="w-full max-w-3xl h-[75vh] flex flex-col justify-center items-center">
              <DialogHeader>
                ...
                <DialogDescription>Arahkan kamera ke QR code/barcode pada tiket untuk check-in.</DialogDescription>
              </DialogHeader>
              <div className="flex flex-col items-center w-full h-full justify-center">
                ...
                {lastScanResult && (
                  ...
                  >
                    ...
                    <button onClick={() => setLastScanResult(null)} className="ml-4 text-xs underline text-white/80">
                      Bersihkan pesan
                    </button>
                  </div>
                )}
      ...
      <Dialog open={bulkCheckoutOpen} onOpenChange={setBulkCheckoutOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            Apakah Anda yakin ingin melakukan checkout massal untuk semua tiket yang sedang difilter?
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setBulkCheckoutOpen(false)} disabled={bulkLoading}>
              Batal
            </Button>
            <Button
              variant="destructive"
              onClick={async () => {
                ...
                try {
                  ...
                  const ids = tickets.filter((t) => t.is_checked_in).map((t) => t.id)
                  if (ids.length === 0) {
                  ...
                  const { error } = await fetch("/api/tickets", {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                    },
                  ...
                  >
                    ...
                    <TableCell className="hidden md:table-cell">
                      <div>
                        ...
                        {ticket.attendee_email && <div className="text-sm text-gray-500">{ticket.attendee_email}</div>}
                      </div>
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
                      <div className="text-sm">{ticket.ticket_type_name || "-"}</div>
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
